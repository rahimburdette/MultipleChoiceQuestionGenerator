...
async function callAnthropicWithRetry(apiKey, body) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    // Convert system prompt to cached format for prompt caching
    let systemPayload = body.system || '';
    if (typeof systemPayload === 'string' && systemPayload.length > 0) {
      systemPayload = [{ type: 'text', text: systemPayload, cache_control: { type: 'ephemeral' } }];
    }
    ...
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({ ...body, system: systemPayload }),
    });
