  ...
  return (
    ...
    >
      <div style={{ padding: '20px 24px' }}>
        ...
        {/* Clinical Image */}
        {q.image_description && (
          <div className="mb-4 rounded-lg overflow-hidden" style={{ border: '1px solid #c7d2fe' }}>
            {q.image_url ? (
              <div>
                <img
                  src={q.image_url}
                ...
                />
                <div style={{ display: 'none' }} className="p-4 bg-indigo-50">
                  <p className="text-[13px] text-gray-700 italic">{q.image_description}</p>
                </div>
                {q.image_source && (
                  <div className="px-3 py-1.5 bg-indigo-50 flex items-center justify-between">
                    <p className="text-[10px] text-gray-500">
                      Source: {q.image_source}
                    </p>
                    {q.image_context_url && (
                      <a href={q.image_context_url} target="_blank" rel="noopener noreferrer" className="text-[10px] text-indigo-500 hover:underline">
                        View source
                      </a>
                    )}
                  </div>
              ...
              <div className="p-4 bg-indigo-50 flex items-start gap-3">
                <Icons.Image />
                <div>
                  <p className="text-[11px] font-bold text-indigo-600 mb-1">Clinical Image</p>
                  <p className="text-[13px] text-gray-700 leading-relaxed italic mb-2">{q.image_description}</p>
                  <div className="flex gap-2">
                    {q.searchLinks.radiopaedia && (
                      <a href={q.searchLinks.radiopaedia} target="_blank" rel="noopener noreferrer"
                        className="text-[11px] font-semibold text-indigo-600 bg-white px-2 py-1 rounded border border-indigo-200 hover:bg-indigo-50">
                        Search Radiopaedia
                      </a>
                    )}
                    {q.searchLinks.openI && (
                      <a href={q.searchLinks.openI} target="_blank" rel="noopener noreferrer"
                        className="text-[11px] font-semibold text-indigo-600 bg-white px-2 py-1 rounded border border-indigo-200 hover:bg-indigo-50">
                        Search OpenI (NLM)
                      </a>
                    )}
