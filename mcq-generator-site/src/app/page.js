...
async function fetchImageForQuestion(q) {
  if (!q.image_description) return q;
  try {
    const res = await fetch('/api/image-search', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ query: q.image_description }),
    });
    if (!res.ok) return q;
    const data = await res.json();
    if (data.images?.length > 0) {
      return { ...q, image_url: data.images[0].url, image_source: data.images[0].source, image_context_url: data.images[0].contextUrl };
    }
    if (data.searchLinks) return { ...q, searchLinks: data.searchLinks };
...
export default function Home() {
  ...
  const generateQuestions = useCallback(async () => {
    ...
    try {
      // Build prompt, optionally append spaced rep review LOs
      let prompt = buildGeneratePrompt(los, numQuestions, difficulty);
      if (learningData?.settings.spacedRepEnabled && dueLOs.length > 0) {
        prompt += buildReviewPrompt(dueLOs);
      }
      ...
      // Step 3: Fetch images
      const hasImages = generated.some((q) => q.image_description);
      if (hasImages) {
        setLoadingStage('Finding clinical images...');
        generated = await Promise.all(generated.map((q) => fetchImageForQuestion(q)));
      }
  ...
  const regenerateQuestion = useCallback(async (idx) => {
    ...
    try {
      ...
      if (newQ[0]) {
        const q = await fetchImageForQuestion(newQ[0]);
        setQuestions((p) => { const n = [...p]; n[idx] = q; return n; });
          ...
          <div className="p-5 sm:p-6">
            ...
            {learningData?.settings.spacedRepEnabled && dueLOs.length > 0 && (
              <div className="mt-3 px-3 py-2 rounded-lg text-[12px] flex items-center gap-2" style={{ background: 'rgba(59,130,246,0.08)', border: '1px solid rgba(59,130,246,0.15)', color: '#93c5fd' }}>
                <Icons.Clock size={12} /> {dueLOs.length} review LO{dueLOs.length > 1 ? 's' : ''} will be included in this generation
              </div>
            )}
